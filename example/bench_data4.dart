...
import "package:tensor_math/src/nd_array_impl.dart" show matMulFloat32x4;
...
void test0(int rows1, int columns1, int rows2, int columns2, bool check) {
  ...
  var a = toFloat32x4List2(rows1, columns1, list1);
  var b = toFloat32x4List3(rows2, columns2, list2);
  ...
  resultArray = matMulFloat32x4(array1, array2);

  var resultList2 = resultArray.reshape(newDimensions: [-1]).toVector();
...
void test(int steps, int rows1, int columns1, int rows2, int columns2) {
  ...
  for (var i = 0; i < steps; i++) {
    matMulFloat32x4(array1, array2);
  }

  print("Finish in ${watch.elapsedMilliseconds} ms");
}

void test2(int steps, int rows1, int columns1, int rows2, int columns2) {
  var list1 =
      new List.generate(rows1 * columns1, (index) => (index + 1).toDouble());
  var list2 = new List.generate(
      rows2 * columns2, (index) => (rows2 * columns2 - index).toDouble());
